...
KS.chr<-function(result.prelim,input.X,window.bed,input.G_k=NULL,region.pos=NULL,tested.pos=NULL,excluded.pos=NULL,M=5,thres.single=0.01,thres.ultrarare=25,thres.missing=0.10,midout.dir=NULL,temp.dir=NULL,jobtitle=NULL,Gsub.id=NULL,impute.method='fixed',bigmemory=T,leveraging=T,LD.filter=NULL){
  ...
  while (start.index<length(region.pos)){
    ...
    # pos<-as.numeric(gsub("^.*\\:","",colnames(G))) #^X8\\.
...
KS.summary<-function(result.window,result.single,M){
  if (is.null(result.window)){
    print("No window result")
